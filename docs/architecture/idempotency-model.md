# Idempotency model: super-model-setup.py

Every install step in `super-model-setup.py` is idempotent. Running the script twice in a row on the same target produces no diffs unless the source ships new content. User edits to user-owned files are preserved.
...
Per `_detect_user_edits` in `super-model-setup.py`:
...
The `tests/foundation/test_super_model_setup_script.py` and `test_super_model_setup_security_hardening.py` suites assert these behaviors with concrete fixtures:
...
Without idempotency, every `super-model-setup` invocation would be a destructive event the user has to think about. With idempotency, it's a no-op safety net - run it whenever you want to refresh the Super-Model surface, with zero risk to your edits.
