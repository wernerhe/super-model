#!/usr/bin/env python3
"""super-mode-setup - install Super-Model into a target project.

Usage:

    python super-mode-setup.py [target-project-dir]
...
if sys.version_info < _MIN_PYTHON:
    sys.stderr.write(
        f"ERROR: super-mode-setup requires Python {_MIN_PYTHON[0]}.{_MIN_PYTHON[1]}+ (you have {sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}).\n"
    )
    sys.stderr.write("Install a recent Python and re-run super-mode-setup.\n")
    sys.exit(1)
...
- `/super-mode-setup` - re-run setup (e.g., to refresh slash commands)
- `/super-visual-debug` - visual fix/improve loop (3D, CAD, UI)
- `/super-mcp-builder` - install / inspect / configure MCP servers

## Configuration

- Per-project config: `.super/config.json` (created by `super-mode-setup`)
