---
name: super-model-setup
description: Run super-model-setup.py to install Super-Model into a target project - CLAUDE.md, .super/config.json, .gitignore entries, MCP wiring, SessionStart hook, install-approval policy, and the 7 slash command shims for Claude Code + Windsurf + Cline (VS Code).
---

# super-model-setup

Orchestrates the per-project install of Super-Model. The LOGICAL flow lives here in module bodies; the actual filesystem work lives in `super-model-setup.py` at the repo root.
...
- The user runs `/super-model-setup` (or invokes `super-model-setup.py` / `.bat` / `.sh` directly).
...
`super-model-setup.py` refuses to run if the parent of its own directory resolves to a drive root (`C:\`, `/`). The fail-fast message tells the user to drop the Super-Model folder INSIDE a project directory.
...
- Tests under `tests/foundation/test_super_model_setup_modules.py` assert manifest entries map 1:1 to module files.
- Two heavier tests (`test_super_model_setup_script.py`, `test_super_model_setup_security_hardening.py`) exercise the script behavior end-to-end.
