...
The install-approval policy is a set of 20 canonical Bash command patterns that should require user approval before execution - typically package-manager invocations (`pip install`, `npm install`, `cargo install`, `apt install`, etc.). These are listed in `CANONICAL_INSTALL_RULES` inside `super-model-setup.py`.
