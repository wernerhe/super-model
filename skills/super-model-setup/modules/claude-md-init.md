...
On first install: creates `<project>/CLAUDE.md` if absent. Content seeded from `CLAUDE_MD_TEMPLATE` in `super-model-setup.py`. The template references the absolute path to the Super-Model source directory and tells Claude to read it.
